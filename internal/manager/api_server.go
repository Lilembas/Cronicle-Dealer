...
func (s *APIServer) setupRoutes() {
	...
	nodesAdmin.Use(s.adminMiddleware())
	{
	...
	strategiesAdmin := protected.Group("/strategies")
	strategiesAdmin.Use(s.adminMiddleware())
	{
		strategiesAdmin.POST("", s.createStrategy)
		strategiesAdmin.POST("/validate", s.validateFormula)
	...
	protected.PUT("/user/password", s.changePassword)

	adminUsers := admin.Group("/admin/users")
	{
		adminUsers.GET("", s.listUsers)
		adminUsers.GET("/:id", s.getUser)
		adminUsers.POST("", s.createUser)
		adminUsers.PUT("/:id", s.updateUser)
		adminUsers.DELETE("/:id", s.deleteUser)
	}

	adminCategories := admin.Group("/admin/categories")
	{
		adminCategories.GET("", s.listCategories)
...
func (s *APIServer) listNodeTags(c *gin.Context) {
	...
	for _, node := range nodes {
		...
		if strings.Contains(node.Tags, "manager") {
			continue
		}
		var tags []string
